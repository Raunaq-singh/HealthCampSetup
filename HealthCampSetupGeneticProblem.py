...
class HealthCampSetupGeneticProblem:
    ...
    def __init__(
        ...
        # Input Data
        self.campSitesFixedCosts = campSitesFixedCosts
        self.campSiteToPatientCost = campSiteToPatientCost
        self.totalCampSites = self.campSiteToPatientCost.shape[0]
        self.totalPatients = self.campSiteToPatientCost.shape[1]
        ...
        self.population = np.random.choice(a=[True, False], size=(self.populationSize, self.totalCampSites), p=[0.5, 0.5])
        self.offsprings = np.empty((self.totalOffsprings, self.totalCampSites))
        ...
        self.bestIndividual = HealthCampSetupGeneticProblem.MAX_FLOAT
    ...
    def calculateScore(self, individualIndex=None, individual=None, cached=True):
        ...
        openCampsites = np.where(individual == True)[0]
        if openCampsites.shape[0] == 0: 
            return HealthCampSetupGeneticProblem.MAX_FLOAT
        score = np.sum(np.amin(self.campSiteToPatientCost[openCampsites, :], axis=0))
        score += self.campSitesFixedCosts.dot(individual)
    ...
    def uniformCrossoverOffspring(self, indexA, indexB):
        crossoverMask = np.random.choice(a=[True, False], size=(self.totalCampSites,), p=[self.crossoverMaskRate, 1-self.crossoverMaskRate])
    ...
    def mutateOffsprings(self):      
        ...
        mask =  np.random.choice(a=[True, False], size=(self.totalOffsprings, self.totalCampSites), p=[mutationRate, 1-mutationRate])
    ...
    def bestSiteForEachPatient(self, individualIndex=0):
        openCampsites = np.where(self.population[individualIndex, :] == True)[0]
        bestSitesForEachPatient = []
        for patientIndex in range(self.totalPatients):
            openCampsiteCosts = self.campSiteToPatientCost[openCampsites, patientIndex]
            chosenCampsiteIndex = np.where(openCampsiteCosts == np.min(openCampsiteCosts))[0][0]
            bestSitesForEachPatient += [openCampsites[chosenCampsiteIndex]]
        return bestSitesForEachPatient
    ...
    def run(self):
        ...
        while not self.finish():
            ...
            self.generation += 1
        self.bestbestSitesForEachPatient = self.bestSiteForEachPatient(0)
